...
/// top-level `code/` folder containing `rats/` and `momentum/`, each of
/// which is itself a workspace of packages) where the packages you actually
/// want to reach are more than one level down.
///
...
mod tests {
    ...
    fn recursive_walk_finds_directories_at_every_depth() {
        ...
        // folder (e.g. `code/`) containing workspace folders (`rats/`,
        // `momentum/`), each of which is itself a workspace of packages
        // (`rat-nest/`, `moments/`) one level further down.
        let dir = tempfile::tempdir().unwrap();
        ...
        assert_eq!(
            names_of(result),
            vec!["moments", "momentum", "rat-nest", "rats"]
        );
    }
    ...
    fn recursive_walk_prunes_directories_excluded_by_skip() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join("rats/node_modules/some-dep")).unwrap();
        std::fs::create_dir_all(dir.path().join("momentum/moments")).unwrap();
        ...
        assert_eq!(names_of(result), vec!["moments", "momentum", "rats"]);
    }
