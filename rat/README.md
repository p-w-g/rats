...
folder. If your `code/` folder contains `rats/` and `momentum/`, and each of
those is itself a workspace of packages (`rats/rat-nest/`,
`momentum/moments/`), plain `fep` run from `code/` never reaches
`rat-nest/` or `moments/` - only `rats/` and `momentum/` themselves. Add
`--recursive` (or its short alias `--r`) to walk the whole subtree instead,
running the command in every matching directory at every depth:
